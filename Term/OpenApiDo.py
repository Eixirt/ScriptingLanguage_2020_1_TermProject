...
def getSidoCode(Do):
    ...
    conn.request("GET","/openapi/service/rest/KorService/areaCode?serviceKey=2anybj3QVonkHWCDnuvtKc%2BODpt7fk2eYr7bH49dUkB%2BD%2FzHkoXLVTgiJRJJJReH8sKK4S3vUFPQzNQZhQjbXg%3D%3D&&MobileOS=ETC&MobileApp=AppTest&numOfRows=17")
    res = conn.getresponse()
    tree = ElementTree.fromstring(res.read().decode('utf-8'))

    itemElement = tree.getiterator("item")
    for item in itemElement:
        if item.find("name").text == Do:
